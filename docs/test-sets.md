...
To just run a test with its default configuration, simply provide an instance
...
Use `python runset.py <setname> --run` to run a runset.

**DEV NOTE:** right now the testconfig iceboot settings are overridden by the
framework with a *hardcoded* value of `localhost` when running run sets. If
possible, please run this assuming the degg board is at `localhost:5012`. If
not, contact maintainer. 
